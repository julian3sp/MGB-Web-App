...
const MapEditor: React.FC<MapEditorProps> = ({ onMapReady }) => {
    ...
    const {
        data: nodesDataFromAPI,
        isLoading: isNodesLoading,
        refetch: refetchNodes,
    } = trpc.getAllNodes.useQuery();
    const {
        data: edgesDataFromAPI,
        isLoading: isEdgesLoading,
        refetch: refetchEdges,
    } = trpc.getAllEdges.useQuery();
    ...
    const tutorialGifs = [addNode, removeNode, removeEdge, createEdge];
    ...
    useEffect(() => {
        ...
        const buildingKey =
            selectedHospital === 'MGB (Chestnut Hill)'
                ? 'chestnut'
    ...
    function displayNodes() {
        ...
        const newStatics = createMarkers(
            map,
            markerLib,
    ...
    function handleEdgeClick(node: Node) {
        ...
        if (!startNodeRef.current) {
            ...
            if (startNodeRef.current.building !== node.building) {
                ...
                startNodeRef.current.building === node.building &&
                startNodeRef.current.floor !== node.floor
            ) {
    ...
    const handleSubmit = async () => {
        ...
        const [nodesRes, edgesRes] = await Promise.all([refetchNodes(), refetchEdges()]);
    ...
    return (
        <div className="flex h-screen">
            <PageWrapper
                contents={
                    <div className="w-full p-5 border-r border-gray-300 flex flex-col gap-4 h-full overflow-y-auto">
